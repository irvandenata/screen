...
<div class="row">
    <div class="col-12">
        <div class="card-box table-responsive">
            <div class="row my-3">
                <div class="col-5">
                    <select class="activatedScreen" id="screen" name="screen">


                        <option disabled>--Pilih Item--</option>

                        @foreach($screen as $item)
                            <option value="{{ $item->id }}" @if ($item->status==1)selected
                        @endif>{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-2">
                    <div class="btn btn-success rounded" onclick="activateScreen()">
                        activate
                    </div>
                </div>
...
@push('script')
    ...
    <script>
        ...
        function setData(result) {
            ...
            $('input[name=leader]').val(result.leader);
        }
